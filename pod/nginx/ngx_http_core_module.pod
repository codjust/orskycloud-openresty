...
C<error_page> directives are inherited from the previous
level only if there are no C<error_page>
directives defined on the current level.
A C<uri> value can contain variables.
...
If C<uri> processing leads to an error,
...
L<limits|http://tools.ietf.org/html/rfc7413#section-5.1>
...
receiving the
L<same SYN packet with data|http://tools.ietf.org/html/rfc7413#section-6.1> more than once.
...
(C<any>) of the
L<ngx_http_access_module|ngx_http_access_module>,
L<ngx_http_auth_basic_module|ngx_http_auth_basic_module> or
